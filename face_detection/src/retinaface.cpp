...
static int post_process_retinaface(rknn_app_context_t *app_ctx, image_buffer_t *src_img, rknn_output outputs[], retinaface_result_t *result, letterbox_t *letter_box) {
    ...
    for (int i = 0; i < validCount; ++i) {
        ...
        result->objects[last_count].box.left   = (int)(clamp(x1, 0, model_in_w) / letter_box->scale); // Face box
        result->objects[last_count].box.top    = (int)(clamp(y1, 0, model_in_h) / letter_box->scale);
        result->objects[last_count].box.right  = (int)(clamp(x2, 0, model_in_w) / letter_box->scale);
        result->objects[last_count].box.bottom = (int)(clamp(y2, 0, model_in_h) / letter_box->scale);
        result->objects[last_count].score = props[i]; // Confidence
        ...
        for (int j = 0; j < 5; ++j) { // Facial feature points
            ...
            result->objects[last_count].landmarks[j].x = (int)(clamp(ponit_x, 0, model_in_w) / letter_box->scale);
            result->objects[last_count].landmarks[j].y = (int)(clamp(ponit_y, 0, model_in_h) / letter_box->scale);
        }
        last_count++;
    }
